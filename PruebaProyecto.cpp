...
//se crea el objeto de tipo HashBlood
  HashBlood bodega;
  vector<string> intr;

// se lee el archivo y se extrae la informacion para insertarla en el objeto HashBlood
  ...
  in.close();
//insertando la informacion en el ojeto HashBlood
  ...
  if(ans=="si"){
      ...
      while(t){
        ...
        if(stoi(ll[1])>bodega.get_blood(ll[0])){
          ...
          bodega.insert(ll[0],bodega.get_blood(ll[0])-stoi(ll[1]));
        }
